...
def _telegram_command_menu_config() -> dict[str, Any]:
    ...
    raw_descriptions = menu_cfg.get("descriptions")
    if isinstance(raw_descriptions, Mapping):
        descriptions = {
            str(k): str(v) for k, v in raw_descriptions.items() if str(v).strip()
        }
    else:
        descriptions = {}
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    capped = all_commands[:max_commands]

    # Operator-supplied description overrides (platforms.telegram.extra.
    # command_menu.descriptions), e.g. to localize a menu entry's text —
    # applied last so it wins regardless of which tier the command came from.
    overrides = _telegram_command_menu_config().get("descriptions") or {}
    if overrides:
        capped = [(n, overrides.get(n, d)) for n, d in capped]

    return capped, hidden_count + hidden_core_count
