...
class GatewayAuthorizationMixin:
    ...
    def _preshared_pairing_codes_for(self, source: "SessionSource") -> Optional[dict]:
        """Load an opt-in pre-shared access-code table for this source's platform.

        Configured via ``platforms.<platform>.extra.preshared_pairing_codes_file``
        (a JSON file mapping an upper-cased code to ``{"name": ..., "active": ...}``).
        Returns ``None`` when not configured, so the default generate-a-code
        pairing flow applies unchanged for every profile that hasn't opted in.
        Returns ``{}`` on a read/parse failure so an unreachable file degrades
        to "no code accepted" rather than crashing the message loop.
        """
        config = getattr(self, "config", None)
        platform = getattr(source, "platform", None)
        if config is None or platform is None or not hasattr(config, "platforms"):
            return None
        platform_cfg = config.platforms.get(platform)
        extra = getattr(platform_cfg, "extra", None) if platform_cfg else None
        if not isinstance(extra, dict):
            return None
        path = extra.get("preshared_pairing_codes_file")
        if not path:
            return None
        import json
        try:
    ...
    def _outbound_policy_rules_for(self, source: "SessionSource"):
        """Load this source's opt-in outbound policy-scan rules, if configured.

        Configured via ``platforms.<platform>.extra.outbound_policy_rules_file``
        (see ``gateway.outbound_policy_scan`` for the rules-file shape).
        Returns ``None`` when not configured — every profile that hasn't
        opted in is completely unaffected. Returns ``None`` on a missing or
        broken rules file too (fails open on config errors, never crashes
        the delivery path — but note the scan itself failing open just means
        "no rules loaded", not "secret text gets through a loaded rule").
        """
        config = getattr(self, "config", None)
        platform = getattr(source, "platform", None)
        if config is None or platform is None or not hasattr(config, "platforms"):
        ...
        if not isinstance(extra, dict):
            return None
        path = extra.get("outbound_policy_rules_file")
        if not path:
            return None
        from gateway.outbound_policy_scan import load_policy_rules
        return load_policy_rules(path)

    def _sync_staff_roster_telegram_id(self, name: str, user_id: str) -> None:
        """Thin wrapper — see ``gateway.pairing.sync_staff_roster_telegram_id``
        for the actual (profile-agnostic, CLI-and-gateway-shared) logic.
        """
        from gateway.pairing import sync_staff_roster_telegram_id
        sync_staff_roster_telegram_id(name, user_id)

    def _resolve_display_name_override(self, source: "SessionSource") -> Optional[str]:
        """For profiles using pre-shared access codes (``_preshared_pairing_codes_for``),
        the real name recorded at approval time must always win over whatever a
        person's Telegram profile happens to display. A phone can be registered
        under any nickname; once someone has verified via their own personal
        code, the agent must address and refer to them as who they actually
        are on file, in every reply and every write-back — not by a nickname
        that has nothing to do with their real identity. Returns None when
        this profile doesn't use pre-shared codes, or the sender isn't a
        recognized approved user (yet).
        """
        preshared = self._preshared_pairing_codes_for(source)
        if preshared is None:
            return None
        pairing_store = self._pairing_store_for(source)
        if pairing_store is None:
            return None
        platform = getattr(source, "platform", None)
        platform_name = platform.value if platform else None
        if not platform_name or not source.user_id:
            return None
        for entry in pairing_store.list_approved(platform_name):
            if pairing_store._user_ids_match(platform_name, entry.get("user_id", ""), source.user_id):
