...
def sync_staff_roster_telegram_id(name: str, user_id: str) -> None:
    """Best-effort write-back: once a real approval finalizes for a pre-shared
    access-code identity, record the real Telegram ID in
    ``Permanent/staff-roster.md``'s last column for the matching name, so the
    vault — not a separate per-bot approval file — stays the accurate answer
    to "who's actually paired in."

    Callable from both the gateway (a live approval during a conversation)
    and the CLI (``hermes pairing approve``, the normal finalization point
    once two-step verification is in effect) — deliberately a plain module
    function rather than a method, since the CLI path has no GatewayRunner
    instance to hang it off of.

    Scoped to the public roster only: the staff bot's own process
    deliberately has no ``CONFIDENTIAL_VAULT_PATH`` set, so there is nothing
    to accidentally write there. Never raises — a write failure here must
    not block the approval that already succeeded.
    """
    if not name:
        return
    ...
    if not vault_path:
        return
    roster_path = os.path.join(vault_path, "Permanent", "staff-roster.md")
    try:
    ...
    for i, line in enumerate(lines):
        ...
        if len(cells) < 5 or cells[1] != name:
            continue
        new_id_cell = f"`{user_id}`"
        if cells[-1] != new_id_cell:
            cells[-1] = new_id_cell
            lines[i] = "| " + " | ".join(cells) + " |\n"
            changed = True
        break
