...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        # Append a random tip to the reset message. Suppressed for client-facing
        # safia-* profiles (Safia Cake & Bakery deployment) -- these tips are
        # Hermes power-user/config guidance, not appropriate for an external
        # business user. Scoped by profile name so other profiles are unaffected.
        try:
            from hermes_constants import get_hermes_home
            from pathlib import Path as _Path
            _is_safia = _Path(get_hermes_home()).name.startswith("safia-")
        except Exception:
            _is_safia = False
        if _is_safia:
            _tip_line = ""
